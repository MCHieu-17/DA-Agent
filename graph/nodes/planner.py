...
def planner_node(state: DataAgentState):
    ...
    return {"plan": [step.model_dump() for step in result.steps], "assumptions": result.assumptions, "current_step_idx": 0, "retry_count": 0, "tool_retry_count": 0, "replan_count": state.get("replan_count", 0) + int(is_replan), "execution_status": None, "validation_feedback": None, "code": None}
>>>>>>> restore-work
